  ...
  const handleBookmark = async (tvSeriesId, tmdbId) => {
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_API_URL}/api/addbookmark/tvseries`,
      ...
      const errorMessage = error.response?.data?.error || 'Failed to bookmark TV Series';
              ...
              <>
                <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-1">
                  {tvSeries.map(series => {
                    ...
                    const date = series.first_air_date;
                    ...
                    return (
                      <div key={series.id} className="p-2 rounded-lg shadow-md relative">
                        <div style={{ position: 'relative', paddingBottom: '100%' }}>
                          <Link to={`/DetailInfoPage/${series.id}?type=tv`}>
                            <img 
                          ...
                          <div className="absolute top-0 right-0 m-2">
                            <button 
                              onClick={() => handleBookmark(series.id, series.tmdbId)} 
