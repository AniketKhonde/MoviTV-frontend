...
const TvSeriesPage = () => {
  ...
  const fetchTvSeries = async () => {
    try {
      const response = await axios.get('https://movitv-backend.onrender.com/api/tvseries');
  ...
  const handleBookmark = async (tvSeriesId,tmdbId) => {
    try {
      const token = localStorage.getItem('token');
      ...
      await axios.post(
        'https://movitv-backend.onrender.com/api/addbookmark/tvseries',
        { userId, tvSeriesId,tmdbId },
        {
          headers: {
      ...
      alert('TV series bookmarked successfully!');
    } catch (error) {
          ...
          <div className="container mx-auto">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-1">
              {tvSeries.map(series => {
                ...
                const date = series.release_date || series.first_air_date;
                // Truncate overview to 10 words
                const overviewWords = series.overview.split(' ');
                const truncatedOverview = overviewWords.length > 10
                  ? overviewWords.slice(0, 10).join(' ') + '...'
                  : series.overview;

                return (
                  <div key={series._id} className="p-2 rounded-lg shadow-md relative">
                    <div className="relative" style={{ paddingBottom: '100%' }}>
                    <Link to={`/DetailInfoPage/${series._id}?tmdbId=${series.id}&type=${series.type}`}>
                      <img src={`https://image.tmdb.org/t/p/w500${series.backdrop_path}`} alt={seriesTitle} className="absolute top-0 left-0 w-full h-full object-cover rounded-lg mb-4 hover:scale-105 transition-transform duration-300"/>
                     </Link>
                      <div className="absolute top-0 right-0 m-2">
                        <button onClick={() => handleBookmark(series._id,series.id)} className="text-white bg-gray-800 rounded-full p-1 hover:bg-gray-600">
                    ...
                    <div className="flex items-center">
                      <h3 className="text-base md:text-lg lg:text-xl font-bold text-white">{seriesTitle}</h3>
